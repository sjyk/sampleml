...
Specifically, \sys applies: (1) a gradient-based update derived from small batches of cleaned data, (2) weighted sampling to carefully select these batches while avoiding biases, (3) optimizations to select records that are most likely to be dirty.
...
Problem (1) is addressed with the gradient-based update.
...
This was a source of significant confusion in the initial submission, and we have clarified the differences.
...
In contrast to the referenced related work, we address the problem of corrupted training data affecting predictive models not using machine learning to facilitate data cleaning. 
\sys allows a user to train predictive models that approximate the true variable relationships in the clean data without cleaning the entire dataset.
