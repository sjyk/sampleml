...
Machine learning has been proposed to improve the efficiency and reliability of data cleaning~\cite{yakout2013don,gokhale2014corleone}.
The salient feature of existing approaches is a Data Cleaning Model (DCM) that learns the effect of a data cleaning operation.
DCMs can improve the efficiency of expensive manual or crowdsourced cleaning by avoiding 
records where the cleaning can be predicted with high certainty~\cite{gokhale2014corleone, DBLP:journals/pvldb/MozafariSFJM14, DBLP:journals/pvldb/YakoutENOI11}.
To improve reliability, a DCM trained on previously cleaned data can be used to evaluate the likelihood of a proposed replacement value \cite{yakout2013don}.
...
\sys addresses the problem of training user-specified models after data cleaning.
We call such models Downstream Models (DSMs) to contrast with the DCMs in prior work.
Unlike DCMs which predict the effect of a data cleaning operations, DSMs model relationships in the clean data.
An example of a DSM is a movie recommender system that is trained on dirty user preference data resulting in error-prone predictions after data cleaning.
A DSM is independent of the errors present in the dataset and the method used to clean those errors.
The DSM problem is more general than the DCM problem with a broader class of allowed models.
DSMs have additional correctness concerns and naive progressive cleaning solutions can lead to arbitrarily incorrect results (Figure \ref{update-arch1}).
This requires a new algorithm to allow for progressive model updates while preserving correctness.
...
We describe related work in the background section (Section \ref{alrw}):
