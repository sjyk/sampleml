...
\sys couples data cleaning with model training to leverage information from the model to identify records that maximally improve a user's model.
The paper shows that naive integration of data cleaning and model training can lead to inaccuracy, and presents several novel techniques for model training on partially clean data.
Specifically, \sys applies a: (1) a gradient-based update derived from small batches of cleaned data with provable guarantees, (2) weighted sampling to carefully select these batches while avoiding biases, (3) optimizations to select records that are most likely to be dirty.

\sys is similar to active learning as both techniques seek to improve the efficiency of expensive manual or crowdsourced data cleaning by carefully selecting samples.
Consequently, the reviewers asked whether traditional active learning could be applied to address the problem of prioritizing data cleaning for user-specified modeling tasks. 
%There are two major limitations for active learning: correctness and efficiency. 
Firstly, in terms of correctness, active learning is not designed for application on mixtures of clean data and not-yet-cleaned dirty data.
Due to the well-studied Simpson's paradox, this can lead to severe inaccuracy (Section \ref{correctness}).
Even if the correctness problem is addressed, in terms of efficiency, the new problem setting leads to several new opportunties for optimizations, which empirically improve on typical active learning critera by up to an order of magnitude (Section \ref{eval}).
...
Yes, the integration with gradient descent allows for provable guarantees on the model's convergence.
We have revised the paper to intuitively motivate this problem in Section \ref{intuit}, formally describe the problem in Section \ref{updp}, and simplified the presentation of the gradient-based update in Section \ref{geod}.
...
We have revised the paper to decouple two subproblems: (1) the correctness problem of how to update a dirty model after cleaning, and (2) the efficiency problem of how to prioritize cleaning using the downstream model. 
Response \textbf{M2b} addresses the correctness problem (1).
The solution to problem (2) is similar to active learning and its implementation is orthogonal to SGD.
...
The architecture section now only describes the essential data flows of the system and is orthogonal to the solutions problems described in Section \ref{statements}.
We also clarify essential components of the architecture that are needed for a minimum viable solution, and then highlight the optional optimizations.
...
We have added examples to each of the technical sections. Section 4 (Architecture) ends with an intuitive end-to-end running example without technical details (Example \ref{archex}).
...
We have added Section \ref{alrw} to connect \sys to relaed work that applies machine learning to data cleaning.
This was a source of significant confusion in the initial submission so we have clarified the differences between using machine learning for data cleaning, and training models after data cleaning.
...
\sys explores a different problem than the referenced related work, namely, the problem of corrupted training data affecting predictive models. 
\sys allows a user to train a predictive models that approximates the true variable relationships in the clean data without cleaning the entire dataset.
The natural question is if frameworks proposed in prior work can apply to this new problem setting, and we added Section \ref{alrw} to answer this:
...
For (1), existing approaches are designed training on homogenous data, i.e., that are previously cleaned or known to be clean.
However, training on a mixture of yet-to-be cleaned dirty data and clean data is unavoidable in the progressive cleaning setting, and without proper care, this can result in severe inaccuracies. 
...
Even if the correctness problem is addressed, the downstream model problem leads to several new opportunties for optimizations (challenge (2)), which empirically improve on typical active learning critera by up to an order of magnitude (Section \ref{eval}).}
...
Section 7 now describes optimizations to this algorithm.
