...
The DSM problem is more general than the DCM problem with a broader class of allowed statistical models, that~\reminder{that $-->$ which?} is not just value-prediction for data cleaning operations.
...
\reminder{I am still a little worried about some sentences in this paragraph (as shown below). The following is the flow that I thought. I am not sure if it makes sense or not.} 

\reminder{We re-organized the paper to first describe a solution to problem (1). In Section 2.3, we present two simple approaches to address this problem. One is to update a dirty model to the model that is trained only on cleaned data; The other is to update it to the model trained on a mix of cleaned and dirty data. We intuitively explained their limiations in Section 2.3, thus propose a new model updating algorithm and prove its corrnectness using SGD theories in Section 5. We compared the performance of these approaches in the experimental section. }
...
Section \ref{model-update} describes one approach to address this problem by applying Stochastic Gradient Descent (SGD)~\reminder{Did we apply SGD? I think we only use SGD to analyze our model updating algorithm, and to understand why it is correct.}.
...
In principle, any incremental optimization technique with similar guarantees could be applied here such as an incremental proximal method or closed forms in simple cases~\reminder{This sentence is a little vague. What are the incremental proximal method and the ``closed forms in simple cases"?}.
...
We describe a number of cases when these optimizations are possible. \reminder{Where is Section 7.2? I think section numbers might mess up }
...
The architecture in Section \ref{sysover} describes how each the components \reminder{each the components?} fit together and the data flows of the system.
...
The DSM problem is more general than the DCM problem with a broader class of allowed statistical models, which~\reminder{which $-->$ that?} is not just record value-prediction for data cleaning operations.
For example, a ``label" in a DSM may be a function of multiple dirty and clean attributes~\reminder{I did not get this example.}.
...
\reminder{The following relationship sounds a little weird. Is there a particular reason to include this paragraph?}\\
...
We could~\reminder{add ``virtually" here?} create a relation $R \times R$ with all pairs of records and an extra missing attribute corresponding to the label. 
...
Prior work has focused \reminder{on?} the problem of designing data structures and algorithms to apply data cleaning progressively.
This is challenging because many data cleaning algorithms require information from entire relations, and designing incremental models is a challenging problem~\reminder{challenging is repetitive. Remove ``and designing incremental models is a challenging problem"?}.
...
Existing work does not explore cleaning driven by the downstream machine learning ``queries"~\reminder{queries $-->$ models?} studied in this work.}
...
\reminder{Would be good to use an consistent representation for ``predictive models" and  ``statistical models" }
...
The challenge is that models trained on partially cleaned data can be arbitrarily incorrect requiring a new algorithms~\reminder{algorithm?} for incrementally updating results given newly cleaned data.
...
\noindent We have addressed all of the formatting and copy editing issues. \reminder{the page number for [16] is missing.}
...
However, we noticed a key problem when these approaches are applied before statistical model training (Section \ref{correctness}). ~\reminder{Might be better to give one sentence to describe what the key problem, and then refer reviewers to section 2.3 for more details.}
...
Furthermore, the DSM problem requires an explicit solution~\reminder{Why do we need new solutions? I actually like what you said in the begining of Meta Review Details: ``Furthermore, training a DSM on a mix of dirty and clean data can lead to arbitrarily incorrect results (Figure \ref{update-arch1}), and this requires a new algorithm to allow for progressive cleaning while preserving correctness."  } to the problem of model accuracy, correctness, and semantics. 
