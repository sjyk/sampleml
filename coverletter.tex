...
\begin{enumerate}
\item Sections 1 and 2 clarify the contributions of \sys and its relationship with related work (e.g., \cite{gokhale2014corleone, DBLP:journals/pvldb/YakoutENOI11, yakout2013don}).

\item In Section \ref{dmodel}, we formalize the definition of dirty data and the data cleaning model used.
...
In prior work, Machine learning has been proposed as a technique to improve the efficiency and reliability of data cleaning~\cite{yakout2013don,gokhale2014corleone}.
...
In contrast, \sys addresses the problem of statistical modeling after data cleaning.
...
We propose several novel extensions to active learning, including dirty data detection and estimation, to clean data to maximally benefit a DSM.
...
Section \ref{correctness} describes two straight-forward integrations of progressive data cleaning and predictive modeling, and how these approaches can lead to error-prone models.
To address this problem, we describe two subproblems: (1) the correctness problem of how to update a dirty model after cleaning and (2) the efficiency problem of how to prioritize cleaning using the downstream model. 
...
Our experiments evalaute all of these design decisions (Section \ref{eval}).
