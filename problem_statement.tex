...
\sys applies the user specified cleaning function $C(\cdot)$ to the sample $S_{clean} = C(S)$.
Given newly cleaned data $S_{clean}$ and a dirty model $\theta^{(d)}$, the model update problem is to calculate $\theta^{new}$. 

$\theta^{new}$ will have some error with respect to the true model $\theta^{(c)}$ (the model if the entire dataset is clean):
\[
