...
If corrupted records are relatively rare, a sampling-based approach might be very inefficient.
...
In the data cleaning literature, error detection (determine whether a record is erroneous) and error repair (synthesize a set updates to remove all erroneous records) are considered two distinct problems~\cite{DBLP:series/synthesis/2012Fan, Dasu:2003:EDM:861869, rahm2000data}.
Error detection is often considered a substantially easier, since one can often declare a set of integrity rules on a database (i.e., a key must be unique), and select rows that violate those rules.
We discuss how rule-based detectors can be integrated with \sys, as well as learning a detector as the dataset is cleaned.
...
Let $r$ be a record in $R$. An detector is a deterministic program that returns a Boolean of whether the record is dirty and a set of columns $e_r$ that are dirty.
\[
D(r) = (\{0,1\}, e_r)
\]
...
We will consider two types of detectors in this paper: exact rule-based detectors that rely on detecting integrity constraint or functional dependency violates, and approximate adaptive detectors that learn which data are likely to be dirty.

\subsection{Rule-Based Detector}
...
This paper focuses on integrity constraints (ICs), conditional functional dependencies (CFDs), and matching dependencies (MDs) where it is efficient to enumerate the set of records that violate at least one constraint. 
...
Rule-based detection is not possible in all cases, especially in cases where the analyst selective modifies data.
