...
We evaluate \sys on real and synthetic datasets for both test and training error.
...
\noindent\textbf{Robust Logistic Regression \cite{feng2014robust}. } Machine Learning has broadly studied a number of \emph{robust} methods to deal with some types of outliers. In particular, this field studies random high-magnitude outliers and techniques to make statistical model training agnostic to their presence. Feng et al. proposed a variant of logistic regression that is robust to outliers. We chose this algorithm because it is a robust extension of the convex regularized loss model, leading to a better apples-to-apples comparison between the techniques. (See details in \cite{activecleanarxiv})  
...
\subsection{\protect\textit{\large A Priori} Detection Scenarios}
This set of experiments evaluate different approaches to cleaning a sample of data compared to \sys using \emph{a priori} detection.
...
PC+D randomly samples data using the dirty data detector, cleans, and writes-back the cleaned data.
...
\subsection{Adaptive Detection Scenarios}
...
This set of experiments evaluate \sys in two real world scenarios, one demonstrating the \emph{a priori} case and the other for the adaptive detection case.
...
To run this experiment, the entire dataset was cleaned up front, and simulated sampling from the dirty data and cleaning by looking up the value in the cleaned data (see \cite{activecleanarxiv} for constraints, errors, and cleaning methodology).
...
This experiment explores the MNIST handwritten digit recognition dataset with a MATLAB image processing pipeline.
