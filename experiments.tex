...
On the other hand, the difference between Naive-Sampling and Naive-Mix in Figure~\ref{real}b highlights the effects of only using the cleaned data or naively combining clean and dirty datasets.
...
\sys and Naive-Sampling extrapolate from the cleaned sample (\sys extropolates a gradient), while Naive-Mix considers the entire dirty data which might be much larger.
