...
% the scenarios are affected by systematic error, since dirtiness in the data affects one classification class more than the other\ewu{THIS IS NOT WHAT SYSTEMATIC ERROR MEANS THOUGH, IT'S A POSSIBLE SYMPTOM}.
...
Figure \ref{real}a lots the distance between the current and cleaned model parameters as more records are cleaned. 
We find that \sys converge faster than those trained with the alternatives.
