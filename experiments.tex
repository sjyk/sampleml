...
Figure~\ref{real}b directly shows how \sys rapidly reduces the \ewu{Test error} --- after 2000 records, \sys's test error is within 0.02 of the fully cleaned model.
...
Second, \sys selects records that are both more likely to by dirty (\ewu{Section XX}) and will most improve the model (\ewu{Section XX}).
...
Due to the systematic bias in the data errors (\ewu{Can we easily explain what the bias was?}), \sys is able to identify the dirty records and
