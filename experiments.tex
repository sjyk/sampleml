...
The first scenario uses two movie datasets from IMDB \footnote{\tiny \url{ftp://ftp.fu-berlin.de/pub/misc/movies/database/}} and Yahoo that was published \reminder{REFERENCE WHERE THEY ARE FROM}.
...
The IMDB dataset is larger (\reminder{HOW MANY}), but much dirtier because it was collected from a web scraper.  
...
Figure~\ref{real}b directly shows how \sys rapidly reduces the \reminder{Test error} --- after 2000 records, \sys's test error is within 0.02 of the fully cleaned model.
...
Second, \sys selects records that are both more likely to by dirty (\reminder{Section XX}) and will most improve the model (\reminder{Section XX}).
...
Due to the systematic bias in the data errors (\reminder{Can we easily explain what the bias was?}), \sys is able to identify the dirty records and
...
There are two types of simulated corruptions that mimic corruptions in previous studies(\reminder{REF?}): \texttt{5x5 Removal} deletes a random 5x5 pixel block by setting the pixel values to 0, and \texttt{Fuzzy} blurs the entire image using a 4x4 moving average patch. 
We apply these corruptions to a random 5\% of the images.%, and mimic the random (Fuzzy) vs. systematic corruption (5x5 removal) studied in the previous experiments.
...
\reminder{How did having a pipeline affect dirtiness?}
