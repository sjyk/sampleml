...
We collected the raw unaggregated data and explored whether suspect contributions could be predicted from features.
...
\subsubsection{Progressive Data Cleaning}  
...
\centering
 ...
 \caption{(a) Systematic corruption (translation) in one variable where the dirty data is in brown, the clean data is in green, and their respective best fit lines are in blue. 
...
In progressive data cleaning setting, there are a number of different sources of errors.
...
Given these inputs, the \sys problem is to return the best estimate $\hat{\theta}$ of the clean model for any limit $k$ on the number of times the data cleaning $C(\cdot)$ can be applied.
...
From an algorithmic perspective,  we have to re-weight data to avoid biases and the population mixtures described previously.
