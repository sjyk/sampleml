...
The sampling framework will naturally follow from this update procedure.
...
This section shows that this model update procedure can be interpreted as a Stochastic 
...
\begin{proposition}
For an appropriately chosen learning rate $\gamma_t$, \sys's updater will converge if $\mathbb{E}(g_S)=g^*$.
\label{unbiased}
\end{proposition}
...
\noindent\textbf{ Non-convex losses: } If the loss in non-convex, the updater will converge towards a local minimum rather than the global minimum (See Appendix \ref{non-convex}).
