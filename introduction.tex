...
Such problems are often addressed in time-consuming loop (Figure~\ref{teaser}) 
where the analysis trains a model, 
inspects the model and its predictions to identify potentially erroneous data, 
cleans erroneous data by transforming (black point) or removing (light grey point) data values, 
and re-trains the model on the cleaned data (blue line).
