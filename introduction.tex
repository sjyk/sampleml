...
When data cleaning is expensive, it is desirable to apply it \textbf{progressively}, where users can inspect early results with only $k \ll N$ records cleaned.
Progressive data cleaning enables users to specify a cleaning budget and clean until that budget is reached.
Even without a budget, progressive cleaning is useful as it allows users measure the impact of a potentially costly data cleaning operation without applying it to the entire data.
...
Rather than cleaning before model training, data cleaning can be directly integrated into the training process, so that \sys can ensure provable guarantees such as convergence and error bounds.
...
The cleaner is a user provided data cleaning technique (e.g., Entity Resolution), and \sys provides the remaining components to apply the cleaner progressively.
