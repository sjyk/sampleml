...
\begin{enumerate}
	...
	\item \texttt{dirty\_model $=$ Updater(dirty\_model, sample\_prob, clean\_sample)}
...
\noindent\textbf{Detector (Section \ref{det}). } The first challenge in \sys is dirty data dection. This is key part of our sampling framework. In this step, we select a candidate set of dirty records $R_{dirty} \subseteq R$. We will discuss two techniques to do this: (1) an a priori case, and (2) and adaptive case. In the a priori case, we know which data is dirty in advance. In the adaptive case, we have to train a classifier based on data we have already cleaned to select the dirty data. This serves two purposes: (1) it reduces the variance of our updates because we can cheaply scan over data we know that is clean, and (2) it increases the fraction of actually dirty records in the candidate batch.
