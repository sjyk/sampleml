...
We revist the architecture diagram in Figure \ref{sys-arch}, and formalize
...
There is at most a one-to-one relationship beftween features in the cleaned data and the dirty data.
...
Given the sample of cleaned examples $\{(x_{clean},y_{clean})\}^K_i$ and the sampling probabilites, the next problem that we address is to update the model.
...
We can feed results back to guide and priortize data cleaning adaptively.
...
In this setting, it is relatively inexpensive to query the set of records that violate some contraint, however, automatically repairing the errors is often NP-Hard \cite{DBLP:journals/pvldb/FanLMTY10}.
...
We additionally require tha $\Sigma$ is transitively closed and the constraints are on disjoint attributes.
...
Then, in the error repair step, we could apply a repair that sets the errorneous signal value to its most likely value.
\end{example}
...
We analyze the situition in which this estimate will reduce variance from the uniform sampling case where $p_i = p_j$.
...
In Figure \ref{sys-arch}, we illustrate our system archictecture in contrast to a traditional data cleaning architecture to show how the above problems fit together.
